...
import { updateConsultationStatusAction, addConsultationNoteAction } from "./actions";
import { FOLLOW_UP_STYLES, formatDateTime } from "@/lib/db/format";
import type { FollowUpStatus, FollowUpNote, ConsultationType } from "@/lib/db/types";
  ...
  return (
    <div className="rounded-xl border border-neutral-200 bg-white p-5">
            ...
            <button
              ...
              onClick={() => {
                ...
                startTransition(async () => {
                  const result = await updateConsultationStatusAction(requestId, opt);
      ...
      <p className="text-xs text-neutral-500">
        {TYPE_LABELS[consultationType]} · {email}
        {phone ? ` · ${phone}` : ""}
      </p>
      <p className="mt-1 text-xs text-neutral-400">Requested {formatDateTime(createdAt)}</p>

      {(preferredTime || requestNotes) && (
        <div className="mt-3 rounded-lg bg-neutral-50 px-3 py-2 text-sm text-neutral-600">
          {preferredTime && <p><span className="font-medium">Preferred time:</span> {preferredTime}</p>}
          {requestNotes && <p className="mt-1 italic">&ldquo;{requestNotes}&rdquo;</p>}
        </div>
      )}
      ...
      <div className="mt-4 border-t border-neutral-100 pt-3">
        <p className="text-xs font-medium text-neutral-500 mb-2">Contact notes</p>
        ...
        <form
          ...
          onSubmit={(e) => {
            ...
            startAddingNote(async () => {
              const result = await addConsultationNoteAction(submissionId, noteText);
              if (!result.error) {
                setNoteText("");
              } else {
        ...
        >
          <input
            ...
            placeholder="Add a note (e.g. called, booked for Tue 3pm)"
